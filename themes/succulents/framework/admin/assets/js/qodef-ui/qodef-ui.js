(function($){
			...
			frame: function () {
				...
				this._frame.on('update', function () {
					...
					jQuery.ajax({
						type: "post",
						url: ajaxurl,
						data: "action=succulents_qodef_gallery_upload_get_images&ids=" + ids,
	...
	function qodefInitAjaxForm() {
		$('#qodef_top_save_button').click(function () {
		...
		$(document).delegate(".qodef_ajax_form", "submit", function (a) {
			var b = $(this),
				c = {
					action: "succulents_qodef_save_options"
				};
			...
			qodefImportBtn.on('click', function (e) {
				...
				if (confirm(confirmMessage)) {
					...
					if (import_opt === 'content') {
						for (var i = 1; i < 10; i++) {
							var str = i < 10 ? 'succulents_content_0' + i + '.xml' : 'succulents_content_' + i + '.xml';
						...
						for (var i = 1; i < 10; i++) {
							var str = i < 10 ? str = 'succulents_content_0' + i + '.xml' : 'succulents_content_' + i + '.xml';
