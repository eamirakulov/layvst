 ...
 */
if ( ! class_exists( 'SucculentsQodefSidebar' ) ) {
	class SucculentsQodefSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'succulents' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'qodef_sidebar', QODE_ROOT . '/framework/admin/assets/js/qodef-ui/qodef-sidebar.js' );
			wp_enqueue_style( 'qodef_sidebar', QODE_ROOT . '/framework/admin/assets/css/qodef-sidebar.css' );
		}
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_html__('Enter Name of the new Widget Area', 'succulents') . "' name='qodef-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_html__('Add Widget Area', 'succulents') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'succulents_qodef_custom_widget_args', $args );
