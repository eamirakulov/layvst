...
class SucculentsQodefAuthorInfoWidget extends SucculentsQodefWidget {
	public function __construct() {
		parent::__construct(
			'qodef_author_info_widget',
			esc_html__( 'Select Author Info Widget', 'succulents' ),
			array( 'description' => esc_html__( 'Add author info element to widget areas', 'succulents' ) )
		);
	...
	protected function setParams() {
		$this->params = array(
			...
			array(
				...
				'title' => esc_html__( 'Author Username', 'succulents' )
			)
		);
	}
		...
		<div class="widget qodef-author-info-widget <?php echo esc_html( $extra_class ); ?>">
			<div class="qodef-aiw-inner">
				<a itemprop="url" class="qodef-aiw-image" href="<?php echo esc_url( get_author_posts_url( $authorID ) ); ?>">
					<?php echo succulents_qodef_kses_img( get_avatar( $authorID, 138 ) ); ?>
				</a>
				<?php if ( $author_info !== "" ) { ?>
					<h4 class="qodef-aiw-title"><?php esc_html_e( 'About Author', 'succulents' ); ?></h4>
