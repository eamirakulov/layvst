...
class QodefTwitterWidget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			esc_html__( 'Select Twitter Widget', 'select-twitter-feed' ),
			array(
				'description' => esc_html__( 'Display your Twitter feed', 'select-twitter-feed' )
			)
		);
	...
	private function setParams() {
		$this->params = array(
			...
			array(
				'name'    => 'type_of_widget',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Type', 'select-twitter-feed' ),
				'options' => array(
					'standard' => esc_html__( 'Standard', 'select-twitter-feed' ),
					'slider'   => esc_html__( 'Slider', 'select-twitter-feed' )
				)
			),
			array(
				'name'  => 'user_id',
			...
			array(
				...
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Show Tweet Icon', 'select-twitter-feed' ),
				'options' => array(
		...
		if ( $twitter_api->hasUserConnected() ) {
			...
			if ( $response->status ) {
				if ( is_array( $response->data ) && count( $response->data ) ) { ?>
					<ul class="qodef-twitter-widget <?php echo esc_attr( $holder_classes ); ?>" <?php echo succulents_qodef_get_inline_attrs( $slider_data ); ?>>
						<?php foreach ( $response->data as $tweet ) { ?>
							
							<?php if ( $type_of_widget == 'slider' ) { ?>
								
								<li class="qodef-tweet-holder">
									<div class="qodef-tweet-text" <?php succulents_qodef_inline_style( $twitter_style ); ?>>
