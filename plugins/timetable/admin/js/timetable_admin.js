jQuery(document).ready(function($){
	...
	$("#add_event_hours").click(function(event){
		...
		{
			var detailsDiv = "";
			if($("#tooltip").val()!="" || $("#before_hour_text").val()!="" || $("#after_hour_text").val()!="" || $("#event_hour_category").val()!="" || $("#available_places").val()!="")
			{
				detailsDiv = "<div>";
				if($("#tooltip").val()!="")
					detailsDiv += "<br><strong>Tooltip:</strong> " + $("#tooltip").val();
				if($("#before_hour_text").val()!="")
					detailsDiv += "<br><strong>Description 1:</strong> " + $("#before_hour_text").val();
				if($("#after_hour_text").val()!="")
					detailsDiv += "<br><strong>Description 2:</strong> " + $("#after_hour_text").val();
...
//						$("#event_hours_table #add_event_hours").after("<input type='hidden' id='event_hours_id' name='event_hours_id' value='" + id + "' />");
	...
	$("#event, #event_category, #hour_category, #weekday, #measure, #filter_style, #filter_kind, #timetable_settings [name='time_format'], #timetable_settings [name='time_format_custom'], #hide_hours_column, #hide_all_events_view, #show_end_hour, #hide_empty, #disable_event_url, #text_align, #desktop_list_view, #responsive, #event_description_responsive, #collapse_event_hours_responsive, #colors_responsive_mode, #export_to_pdf_button, #direction, #event_layout, #timetable_font, #timetable_font_subset, #show_booking_button, #show_available_slots, #allow_guest_booking, #show_guest_name_field, #guest_name_field_required, #show_guest_phone_field, #guest_phone_field_required, #show_guest_message_field, #guest_message_field_required").change(function(event, param){
		if(param=="skip")
			return;
		if($(this).attr("id")!="timetable_font" || ($(this).attr("id")=="timetable_font" && param==null))
			generateShortcode();
	});
	$("#filter_label, #filter_label_2, #generate_pdf_label, #available_slots_singular_label, #available_slots_plural_label, #booking_label, #booked_label, #unavailable_label, #booking_popup_label, #login_popup_label, #cancel_popup_label, #continue_popup_label, #row_height, #id, #timetable_font_custom, #timetable_font_size, #booking_popup_message, #booking_popup_thank_you_message, #timetable_custom_css").on('change keyup', function(){
		generateShortcode();
	});
...
//	$("#upcoming_events_time_from").live("change", function(){
	...
	$("#allow_guest_booking").change(function(event, param){
		...
		if(self.val()=="yes")
		{
			...
			$(".show_guest_email_field").removeClass("tt_hide");
			$(".guest_email_field_required").removeClass("tt_hide");
...
{
	var $ = jQuery;
	var content = (typeof(tinyMCE.editors[editor_id])!=="undefined" && !tinyMCE.editors[editor_id].isHidden() ? 
		tinyMCE.editors[editor_id].getContent().trim() : $("#" + editor_id).val().trim());
	//Text editor returns content with new lines, while Visual editor doesn't
	//We need to make sure that the returned content is the same
	//independently of the selected editor.
...
{
	var $ = jQuery;
	if(typeof(tinyMCE.editors[editor_id])!=="undefined" && !tinyMCE.editors[editor_id].isHidden())
		tinyMCE.editors[editor_id].setContent(content);
	else
