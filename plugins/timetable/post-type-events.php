...
function timetable_save_events_postdata($post_id) 
{
	...
	if(isset($_POST["weekday_ids"]))
	{
		...
		for($i=0; $i<$hours_count; $i++)
		{
			...
			if(!(isset($_POST["event_hours_ids"][$i]) && $_POST["event_hours_ids"][$i]>0))
			{
				
				$query = $wpdb->prepare("INSERT INTO `" . $wpdb->prefix . "event_hours`(event_id, weekday_id, start, end, tooltip, before_hour_text, after_hour_text, category, available_places, slots_per_user) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", $post_id, $_POST["weekday_ids"][$i], $_POST["start_hours"][$i], $_POST["end_hours"][$i], $_POST["tooltips"][$i], $_POST["before_hour_texts"][$i], $_POST["after_hour_texts"][$i], $_POST["event_hours_category"][$i], $_POST["available_places_array"][$i], $slots_per_user);
			}
			else
			{
				//update only if the available_places is equal or grater than the count of existing bookings
				$booking_count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(booking_id) FROM `" . $wpdb->prefix . "event_hours_booking` WHERE event_hours_id=%d", $_POST["event_hours_ids"][$i]));
				if($_POST["available_places_array"][$i]>=$booking_count)
				{
					$query = $wpdb->prepare("UPDATE `" . $wpdb->prefix . "event_hours` SET weekday_id=%s, start=%s, end=%s, tooltip=%s, before_hour_text=%s, after_hour_text=%s, category=%s, available_places=%s, slots_per_user=%s WHERE event_hours_id=%s", $_POST["weekday_ids"][$i],  $_POST["start_hours"][$i], $_POST["end_hours"][$i], $_POST["tooltips"][$i], $_POST["before_hour_texts"][$i], $_POST["after_hour_texts"][$i], $_POST["event_hours_category"][$i], $_POST["available_places_array"][$i], $slots_per_user, $_POST["event_hours_ids"][$i]);
				}
				else
				{
	...
	if(isset($_POST["delete_event_hours_ids"]))
	{
		...
		if($delete_event_hours_ids_count)
		{
			...
			$wpdb->query("DELETE FROM `" . $wpdb->prefix . "event_hours_booking` WHERE event_hours_id IN(" . implode(",", $_POST["delete_event_hours_ids"]) . ")");
		}
	}
