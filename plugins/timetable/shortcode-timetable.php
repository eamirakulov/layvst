...
function tt_timetable($atts, $content)
{
	...
	if($filter_style=="dropdown_list")
	{
		...
		if($export_to_pdf_button && $responsive)
		{
			...
			$pdf_hover_text_color = (strtoupper($generate_pdf_hover_text_color)!="FFFFFF" ? $generate_pdf_hover_text_color : "");
			$pdf_hover_bg_color = (strtoupper($generate_pdf_hover_bg_color)!="1F736A" ? $generate_pdf_hover_bg_color : "");
			...
			for($j=0; $j<count($weekdays); $j++)
			{
				...
				{	
					if(tt_hour_in_array($i, (isset($event_hours_tt[$weekday_fixed_number]) ? $event_hours_tt[$weekday_fixed_number] : array()), $measure, $hours_min))
					{
						...
						$booking_hover_bg_color = ($booking_hover_bg_color!="" && strtoupper($booking_hover_bg_color)!="07B38A" ? $booking_hover_bg_color : "");
						...
						$row_content .= tt_get_row_content($events, compact("events_page", "time_format", "event_layout", "global_colors", "disable_event_url", "show_booking_button", "show_available_slots", "allow_guest_booking", "booking_label", "available_slots_singular_label", "available_slots_plural_label", "booked_label", "unavailable_label"));
						$row_content .= '</td>';
						$row_empty = false;
					}
					else
...
function prepare_booking_button($args)
{
	...
	if($args['current_user_booking_count']>=$args['slots_per_user'])
	{
