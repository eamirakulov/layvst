...
function timetable_cancel_booking()
{
	...
	{
		timetable_delete_verified_booking($booking_id, $validation_code);
	}
	
	if(count($bookings_ids)==count($validation_codes) && count($bookings_ids)>1)
	{
		...
		for($i=0, $max_i = count($bookings_ids); $i<$max_i; $i++)
		{
			...
			{
				timetable_delete_verified_booking($bookings_ids[$i], $validation_codes[$i]);
			}
		}
	}
...
function timetable_delete_verified_booking($booking_id, $validation_code)
{
	$booking = TT_DB::getBookings(array(
		'booking_id' => $booking_id,
		'validation_code' => $validation_code,
	));
	if(!isset($booking[0]['booking_id']) || $booking[0]['booking_id']!=$booking_id)
	{
		echo '<b>' . sprintf(__('Error: Booking #%d does not exist or validation code is incorrect.', 'timetable'), $booking_id) . '</b><br>';
	}
	else
	{
		//delete booking
		TT_DB::deleteBooking($booking[0]['booking_id']);
		//display booking information
		echo '<b>' . sprintf(__('Booking #%d deleted', 'timetable'), $booking[0]['booking_id']) . '</b><br>';		
		echo sprintf(__('Title: %s', 'timetable'), $booking[0]['event_title']) . '<br>';
		echo sprintf(__('Time: %s', 'timetable'), $booking[0]['start'] . '-' . $booking[0]['end']) . '<br>';
		echo sprintf(__('Column: %s', 'timetable'), $booking[0]['weekday']) . '<br>';
		if($booking[0]['event_description_1'])
			echo sprintf(__('Description 1: %s', 'timetable'), $booking[0]['event_description_1']) . '<br>';
		if($booking[0]['event_description_2'])
			echo sprintf(__('Description 2: %s', 'timetable'), $booking[0]['event_description_2']) . '<br>';
		echo '<br>';
	}
}
...
if(is_admin())
{
	function timetable_admin_menu()
	{
		...
		$shortcode_generator_page = add_submenu_page('timetable_admin', __('Shortcode Generator', 'timetable'), __('Shortcode Generator', 'timetable'), 'manage_options', 'timetable_admin', 'timetable_admin_page', '', 20);
...
<p>Selected event hour doesn't exist.<br>Please select different event.</p>", "timetable");
		...
		//if user isn't logged in, then show additional label 'Got an account? Login'
		if(!$user_id && $booking_form_config['allow_guest_booking']=='yes')
		{
			$result['msg'] .= sprintf(__('<p class="tt_guest_option">Don\'t have an account? <a href="%s">Continue as guest</a></p>', 'timetable'), '#');
			$result['msg'] .= sprintf(__('<p class="tt_login_option tt_hide">Got an account? <a href="%s">Login</a></p>', 'timetable'), wp_login_url($redirect_url));
		}
...
function tt_booking_form($args)
{
	...
	$output .= 
	'<form class="tt_booking_form ' . (is_user_logged_in() && $max_slots ? '' : 'tt_hide') . '">';
	
	$guest_fields_class = 'tt_hide';
	...
	if($args['show_guest_name_field']=='yes')
	{
		...
		$output .= 
		'<div class="tt_field_wrapper tt_hide">
	...
	{
		$output .= 
		'<div class="tt_field_wrapper ' . (is_user_logged_in() && $max_slots ? '' : 'tt_hide') . '" data-max-slots="'.$max_slots.'">
	...
	if($args['show_guest_message_field']=='yes')
	{
		...
		$output .= 
		'<div class="tt_field_wrapper wide tt_hide">
...
//function tt_btn_book($book_label, $login_label, $redirect_url, $allow_guest_booking)
function tt_btn_book($args)
{
	...
	$output = '';
	$output .= '<a href="#" class="tt_btn book ' . (!is_user_logged_in() ? 'tt_hide' : '') . '">' . $args['booking_label'] . '</a>';
	$output .= '<a href="' . wp_login_url($args['redirect_url']) . '" class="tt_btn login ' . (is_user_logged_in() ? 'tt_hide' : '') . '">' . $args['login_label'] . '</a>';
