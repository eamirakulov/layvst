...
jQuery(document).ready(function() {
  var Closed = false;
  jQuery('.ewd-urp-pointer-exit').on('click', function() {
    Closed = true;
  })
});
...
function URP_Dismiss_Pointers() {
  jQuery('.urp-skip-all-tutorial-pop-ups').on('click', function() {
    jQuery.post( ajaxurl, { action: 'urp-dismiss-wp-pointers'});
...
jQuery(document).ready(function($) {
  jQuery('body').on('DOMNodeInserted', function(event) {
    if (jQuery(event.target).hasClass('ewd-urp-pointer-count')) {URP_Dismiss_Pointers();}
  });
});