...
class PortfolioList implements Lib\ShortcodeInterface {
			...
			vc_map( array(
					...
					'params'   => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Click Behavior', 'select-core' ),
							'value'      => array(
								esc_html__( 'Open portfolio single page on click', 'select-core' )   => '',
								esc_html__( 'Open gallery in Pretty Photo on click', 'select-core' ) => 'gallery'
							)
						),
						array(
							'type'        => 'dropdown',
                        ...
                        array(
                            ...
                            'heading'    => esc_html__( 'Enable Text', 'select-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Enable Number of Images', 'select-core' ),
							'value'      => array_flip( succulents_qodef_get_yes_no_select_array( false, true ) ),
							'dependency' => array( 'element' => 'type', 'value' => array( 'gallery' ) ),
							'group'      => esc_html__( 'Content Layout', 'select-core' )
						),
	...
	public function render( $atts, $content = null ) {
		...
		$html = qodef_core_get_cpt_shortcode_module_template_part( 'portfolio', 'portfolio-list', 'portfolio-holder', $params['type'], $params, $additional_params );
	...
	public function getImageSize( $params ) {
		...
		if ( ! empty( $params['image_proportions'] ) && $params['type'] == 'gallery' ) {
			...
			switch ( $image_size ) {
				case 'landscape':
					$thumb_size = 'succulents_qodef_landscape';
					break;
				case 'portrait':
					$thumb_size = 'succulents_qodef_portrait';
					break;
				case 'square':
					$thumb_size = 'succulents_qodef_square';
					break;
				case 'medium':
					$thumb_size = 'medium';
					break;
		...
		if ( $params['type'] == 'masonry' && $params['enable_fixed_proportions'] === 'yes' ) {
			...
			switch ( $fixed_image_size ) {
				...
				case 'large-width-height':
					$thumb_size = 'succulents_qodef_huge';
					break;
				default :
					$thumb_size = 'full';
					break;
	...
	public function getLoadMoreStyles( $params ) {
		...
		if ( ! empty( $params['load_more_top_margin'] ) ) {
			...
			if ( succulents_qodef_string_ends_with( $margin, '%' ) || succulents_qodef_string_ends_with( $margin, 'px' ) ) {
				$styles[] = 'margin-top: ' . $margin;
			} else {
				$styles[] = 'margin-top: ' . succulents_qodef_filter_px( $margin ) . 'px';
			}
		}
	...
	public function getFilterHolderStyles( $params ) {
		...
		if ( ! empty( $params['filter_bottom_margin'] ) ) {
			...
			if ( succulents_qodef_string_ends_with( $margin, '%' ) || succulents_qodef_string_ends_with( $margin, 'px' ) ) {
				...
				$styles[] = 'margin-bottom: ' . succulents_qodef_filter_px( $margin ) . 'px';
			}
		}
	...
	public function getItemLink() {
		...
		return apply_filters( 'succulents_qodef_portfolio_external_link', $portfolio_link );
	}
	...
	public function getItemLinkTarget() {
		...
		return apply_filters( 'succulents_qodef_portfolio_external_link_target', $portfolio_link_target );
	}
