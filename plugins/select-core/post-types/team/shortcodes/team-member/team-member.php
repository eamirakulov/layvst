...
class TeamMember implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
        if(function_exists('vc_map')) {
	        vc_map( array(
			        'name'                      => esc_html__( 'Select Team Member', 'select-core' ),
			        'base'                      => $this->getBase(),
			        ...
			        'params'                    => array(
                        array(
					        ...
					        'heading'    => esc_html__( 'Select Team Member', 'select-core' ),
					        'settings'   => array(
						        'sortable'      => true,
						        'unique_values' => true
					        ),
					        'description' => esc_html__( 'If you left this field empty then project ID will be of the current page', 'select-core' )
				        )
			        )
		        )
	        );
    ...
    public function render($atts, $content = null) {
        ...
        $html = qodef_core_get_cpt_shortcode_module_template_part('team', 'team-member', $params['team_member_layout'], '', $params);
