...
class QodefCoreImport {
	...
	function __construct() {
		...
		$this->importURI       = defined( 'QODE_PROFILE_SLUG' ) ? 'http://export.' . QODE_PROFILE_SLUG . '-themes.com/' : '';
	...
	public function import_options( $file ) {
		$options       = $this->file_options( $file );
		$result        = update_option( 'qodef_options_succulents', $options );
		if( function_exists( 'succulents_qodef_generate_dynamic_css_and_js' ) ) {
			succulents_qodef_generate_dynamic_css_and_js();
		}
	...
	function qodef_admin_import() {
		if ( qodef_core_theme_installed() ) {
			...
			$this->pagehook = add_submenu_page(
				...
				array( $succulents_qodef_Framework->getSkin(), 'renderImport' )
			);
			
			add_action( 'admin_print_scripts-' . $this->pagehook, 'succulents_qodef_enqueue_admin_scripts' );
			add_action( 'admin_print_styles-' . $this->pagehook, 'succulents_qodef_enqueue_admin_styles' );
		}
	}
	...
	function qodef_update_meta_fields_after_import($folder){
		...
		$images_field_array = succulents_qodef_meta_boxes()->getOptionsByType('image');
	...
	function qodef_update_options_after_import($folder){
		...
		$global_options = get_option( 'qodef_options_succulents');
		$new_global_values = str_replace($demo_url, $url, $global_options);
		update_option( 'qodef_options_succulents', $new_global_values);
	...
	function qodef_import_get_demo_url($folder) {
		...
		$demo_url	= 'http://' . $folder_new . '.qodef-themes.com/';
