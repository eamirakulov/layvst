...
if ( ! function_exists( 'qodef_core_get_cpt_shortcode_module_template_part' ) ) {
	 ...
	 * @return html
	 */
	function qodef_core_get_cpt_shortcode_module_template_part( $post_type, $shortcode, $template, $slug = '', $params = array(), $additional_params = array() ) {
		
		//HTML Content from template
		$html          = '';
		$template_path = QODE_CORE_CPT_PATH . '/' . $post_type . '/shortcodes/' . $shortcode . '/templates';
...
if ( ! function_exists( 'qodef_core_get_module_shortcode_template_part' ) ) {
    /**
     * Loads module template part.
     *
     * @param string $module name of the module to load
     ...
     * @return html
     */
    function qodef_core_get_module_shortcode_template_part( $module, $shortcode, $template, $slug = '', $params = array() ) {

        //HTML Content from template
        $html          = '';
        $template_path = QODE_CORE_ABS_PATH . '/' . $module . '/shortcodes/' . $shortcode . '/templates';
...
if ( ! function_exists( 'succulents_qodef_add_user_custom_fields' ) ) {
	/**
	 * Function creates custom social fields for users
	 *
	 * return $user_contact
	 */
	function succulents_qodef_add_user_custom_fields( $user_contact ) {
		/**
	...
	add_filter( 'user_contactmethods', 'succulents_qodef_add_user_custom_fields' );
}
...
if ( ! function_exists( 'qodef_core_set_open_graph_meta' ) ) {
	...
	function qodef_core_set_open_graph_meta() {
		...
		if ( succulents_qodef_option_get_value( 'enable_open_graph' ) === 'yes' ) {
			...
			if ( succulents_qodef_is_woocommerce_installed() && ( function_exists( 'is_shop' ) && is_shop() ) ) {
			...
			if ( ! empty( $id ) ) {
				...
				if ( get_post_thumbnail_id( $id ) !== '' ) {
					...
					$image = succulents_qodef_option_get_value( 'open_graph_image' );
				}
			} else {
				global $wp;
	...
	add_action( 'succulents_qodef_header_meta', 'qodef_core_set_open_graph_meta' );
}