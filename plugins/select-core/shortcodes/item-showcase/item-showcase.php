...
class ItemShowcase implements Lib\ShortcodeInterface {
	...
	public function render( $atts, $content = null ) {
		...
		if ( ! empty( $image_top_offset ) ) {
			$item_image_style = 'margin-top: ' . succulents_qodef_filter_px( $image_top_offset ) . 'px';
		}
		
		$html = '<div class="qodef-item-showcase-holder clearfix">';
			$html .= do_shortcode( $content );
			if ( ! empty( $item_image ) ) {
				$html .= '<div class="qodef-is-image" ' . succulents_qodef_get_inline_style( $item_image_style ) . '>';
