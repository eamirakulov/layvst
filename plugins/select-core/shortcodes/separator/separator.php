...
class Separator implements Lib\ShortcodeInterface {
	...
	private function getHolderStyles( $params ) {
		...
		if ( $params['width'] !== '' ) {
			if ( succulents_qodef_string_ends_with( $params['width'], '%' ) || succulents_qodef_string_ends_with( $params['width'], 'px' ) ) {
				$styles[] = 'width: ' . $params['width'];
			} else {
				$styles[] = 'width: ' . succulents_qodef_filter_px( $params['width'] ) . 'px';
			}
		}
		
		if ( $params['thickness'] !== '' ) {
			$styles[] = 'border-bottom-width: ' . succulents_qodef_filter_px( $params['thickness'] ) . 'px';
		}
		
		if ( $params['top_margin'] !== '' ) {
			if ( succulents_qodef_string_ends_with( $params['top_margin'], '%' ) || succulents_qodef_string_ends_with( $params['top_margin'], 'px' ) ) {
				$styles[] = 'margin-top: ' . $params['top_margin'];
			} else {
				$styles[] = 'margin-top: ' . succulents_qodef_filter_px( $params['top_margin'] ) . 'px';
			}
		}
		
		if ( $params['bottom_margin'] !== '' ) {
			if ( succulents_qodef_string_ends_with( $params['bottom_margin'], '%' ) || succulents_qodef_string_ends_with( $params['bottom_margin'], 'px' ) ) {
				$styles[] = 'margin-bottom: ' . $params['bottom_margin'];
			} else {
				$styles[] = 'margin-bottom: ' . succulents_qodef_filter_px( $params['bottom_margin'] ) . 'px';
			}
		}
