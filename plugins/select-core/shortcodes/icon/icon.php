...
class Icon implements Lib\ShortcodeInterface {
		    ...
		    vc_map(
		    	array(
				    ...
				    'params'                    => array_merge(
					    ...
					    \SucculentsQodefIconCollections::get_instance()->getVCParamsArray(),
					    array(
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'size',
	...
	private function generateIconStyles( $params ) {
		...
		if ( ( $params['type'] !== 'qodef-normal' && ! empty( $params['shape_size'] ) ) || ( $params['type'] == 'qodef-normal' ) ) {
			if ( ! empty( $params['custom_size'] ) ) {
				$iconStyles[] = 'font-size:' . succulents_qodef_filter_px( $params['custom_size'] ) . 'px';
			}
		}
	...
	private function generateIconHolderStyles( $params ) {
		...
		if ( $params['type'] !== 'qodef-normal' ) {
			...
			if ( ! empty( $shapeSize ) ) {
				$iconHolderStyles[] = 'width: ' . succulents_qodef_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'height: ' . succulents_qodef_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'line-height: ' . succulents_qodef_filter_px( $shapeSize ) . 'px';
			}
			...
			if ( ! empty( $params['border_color'] ) && ( isset( $params['border_width'] ) && $params['border_width'] !== '' ) ) {
				...
				$iconHolderStyles[] = 'border-width: ' . succulents_qodef_filter_px( $params['border_width'] ) . 'px';
			...
			if ( $params['type'] == 'qodef-square' ) {
				if ( isset( $params['border_radius'] ) && $params['border_radius'] !== '' ) {
					$iconHolderStyles[] = 'border-radius: ' . succulents_qodef_filter_px( $params['border_radius'] ) . 'px';
				}
			}
		}
