...
class IconWithText implements Lib\ShortcodeInterface {
			...
			vc_map(
				array(
					...
					'params'                    => array_merge(
						array(
							...
							array(
								...
								'value'       => array(
									...
									esc_html__( 'Icon Right From Text', 'select-core' ) => 'icon-right',
						...
						succulents_qodef_icon_collections()->getVCParamsArray(),
						array(
							array(
								'type'       => 'attach_image',
							...
							array(
								...
								'heading'    => esc_html__( 'Title Custom Size', 'select-core' ),
								'dependency' => array( 'element' => 'title', 'not_empty' => true ),
								'group'      => esc_html__( 'Text Settings', 'select-core' )
							),
							array(
                            ...
                            array(
                                'type'       => 'dropdown',
                                'param_name' => 'text_tag',
                                'heading'    => esc_html__( 'Text Tag', 'select-core' ),
                                'value'      => array_flip(succulents_qodef_get_title_tag(true)),
	...
	public function render( $atts, $content = null ) {
		...
		$default_atts = array_merge( $default_atts, succulents_qodef_icon_collections()->getShortcodeParams() );
	...
	private function getIconParameters( $params ) {
		...
		if ( empty( $params['custom_icon'] ) ) {
			$iconPackName = succulents_qodef_icon_collections()->getIconCollectionParamNameByKey( $params['icon_pack'] );
			...
			if ( ! empty( $params['custom_icon_size'] ) ) {
				$params_array['custom_size'] = succulents_qodef_filter_px( $params['custom_icon_size'] ) . 'px';
			}
			...
			if ( ! empty( $params['shape_size'] ) ) {
				$params_array['shape_size'] = succulents_qodef_filter_px( $params['shape_size'] ) . 'px';
			}
			...
			if ( $params['icon_border_width'] !== '' ) {
				$params_array['border_width'] = succulents_qodef_filter_px( $params['icon_border_width'] ) . 'px';
			}
	...
	private function getContentStyles( $params ) {
		...
		if ( $params['text_padding'] !== '' && $params['type'] === 'icon-left' ) {
			$styles[] = 'padding-left: ' . succulents_qodef_filter_px( $params['text_padding'] ) . 'px';
		}
		
		if ( $params['text_padding'] !== '' && $params['type'] === 'icon-top' ) {
			$styles[] = 'padding-top: ' . succulents_qodef_filter_px( $params['text_padding'] ) . 'px';
		}
	...
	private function getTitleStyles( $params ) {
		...
		if ( ! empty( $params['title_custom_size'] ) ) {
			$styles[] = 'font-size: ' . $params['title_custom_size'];
		}
	...
	private function getTextStyles( $params ) {
		...
		if ( $params['text_top_margin'] !== '' ) {
			$styles[] = 'margin-top: ' . succulents_qodef_filter_px( $params['text_top_margin'] ) . 'px';
		}
